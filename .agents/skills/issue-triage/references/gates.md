...
2. Is it available? Do not add a gate to assigned work. Leave existing gates on assignment alone; someone claiming a task is not criteria drift. `agent-in-progress` makes the entire issue read-only.
