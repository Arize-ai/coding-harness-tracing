...
import (
	...
	"github.com/Arize-ai/coding-harness-tracing/cmd/acht/internal/manifest"
)
...
func CheckVenv(opts Options) Verdict {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return Verdict{
				...
				Remediate: "Run `acht install` (or `./install.sh`) to bootstrap the venv.",
			}
		}
		return Verdict{
			Name:      "venv",
	...
	if info.IsDir() {
		return Verdict{
			...
			Remediate: "Delete ~/.arize/harness/venv and re-run `acht install`.",
		}
	}
	return Verdict{
		Name:   "venv",
...
func CheckHarnessSettings(name string, entry manifest.HarnessEntry, opts Options) Verdict {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			remediate := "Run `acht install` to write the settings file."
			if entry.DisplayName != "" {
				remediate = fmt.Sprintf("Run `acht install` to write %s settings, or launch %s once to create defaults.", entry.DisplayName, entry.DisplayName)
			}
			return Verdict{
				Name:      checkName,
				Pass:      false,
	...
	if err := json.Unmarshal(data, &parsed); err != nil {
		return Verdict{
			...
			Remediate: fmt.Sprintf("Open %s and fix the JSON syntax, or re-run `acht install` to regenerate.", path),
		}
	}
	return Verdict{
		Name:   checkName,
...
func CheckHarnessEnv(name string, entry manifest.HarnessEntry, opts Options) Verdict {
	...
	if cfgErr != nil {
		return Verdict{
			...
			Remediate: fmt.Sprintf("Fix YAML syntax in %s or remove it and re-run `acht install`.", cfgPath),
		}
	}
	if len(configHits) > 0 {
		return Verdict{
	...
	return Verdict{
		...
		Remediate: fmt.Sprintf("Run `acht install` or `acht config` to populate %s env vars.", name),
	}
}
