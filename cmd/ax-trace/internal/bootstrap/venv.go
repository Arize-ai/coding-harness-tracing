...
func EnsureVenv(ctx context.Context, opts Options, uvPath string) (venvPython string, reused bool, err error) {
	...
	if venvIsHealthy(ctx, opts, pyPath) {
		fmt.Fprintf(opts.Stdout, "[ax-trace] reusing existing venv at %s\n", venvDir)
	...
	fmt.Fprintf(opts.Stdout, "[ax-trace] creating venv at %s\n", venvDir)
	...
	fmt.Fprintf(opts.Stdout, "[ax-trace] installing coding-harness-tracing into venv\n")
	if err := opts.Runner.Run(ctx, uvPath,
...
func EnsureMacOSSSLFix(ctx context.Context, opts Options, venvPython string) error {
	...
	fmt.Fprintln(opts.Stdout, "[ax-trace] SSL certificates configured via certifi")
	return nil
}
